...
smc_particle_sim_pi = function(input,input_date,loc.seed,mob_mat,
                   unit.samp,day.numb,hospD,testD,pbar,qbar,
                   prob_h,alpha_hosp,alpha_test,MaxdaysH,MaxdaysT)
{
  ...
  #show(hospD[M+1:unit.samp,])
  #show(testD[M+1:unit.samp,])
  #cat("smc_particle_sim: M=",M,"day.numb=",day.numb,"\n")
  if(abs(M-M2)>0)
  {
...
#        insH[M+u-j,1] = insH[M+u-j,1]-insH[M+u-j,2+j]
  ...
  for(i in 1:unit.samp)
  {
    if(!is.na(hospD$N[M+i]))
    {
      ...
      if(hospD$N[M+i]>num[i])
        logw.new =  -9999                #Somewhat ad hoc
      else
        {
          if(is.null(alpha_hosp))
            l = dbinom(hospD$N[M+i],num[i],prob_h[M+i],log=TRUE)
	        else
	          l = dbbinom(hospD$N[M+i],num[i],
                        alpha=alpha_hosp,beta=beta_hosp[M+i],log=TRUE)
	        if(is.na(l))
	          show(c(M+i,hospD$N[M+i],num[i],beta_hosp[M+i]))
          logw.new = logw.new+l
        }
      if(is.na(logw.new))
  ...
  if(!is.null(testD))
  {
    ...
    for(i in 1:unit.samp)
    {
      ...
      ind = 1+c(1:NN)*(N-1)
      #show(c(M+i,testD$N[M+i], ((M+i)%%7)))
      if(!is.na(testD$N[M+i]))
      {       
        ...
        if(is.na(pi.t))
        {
          print("sim_particle_sim: Something is wrong")
          return(NULL)
        }
