...
#' @export
smc_plot_C = function(mat,days,quant=c(0.95,0.90,0.75,0.5),col = "#08769c",
                      title="",trunc=NULL,
                      end_date=end_date,test_date=NULL,pred_date=NULL,log=FALSE)
{
  require(ggplot2)
  require(tidybayes)
  require(data.table)
 ...
 #Wide to long format
  ...
  if(log)
    q = q + scale_y_continuous(name="New I",trans='log10')
  else
    q = q + scale_y_continuous(name="New I")
